...
/**
 * ProsjektKravFlowNode - React Flow node for ProsjektKrav entities
 * Reuses existing EntityCard component for consistent styling and functionality
 */
const ProsjektKravFlowNode = ({ data, selected }) => {
  const { entity, onEntitySelect, onFieldSave, viewOptions } = data;

  return (
    <>
      {/* Input handle (left side) - receives connections from Emne */}
      <Handle
        type="target"
      ...
      {/* Output handle (right side) - connects to ProsjektTiltak */}
      <Handle
        type="source"
      ...
      <div className="bg-white border-2 border-gray-200 rounded-lg shadow-sm hover:shadow-md transition-shadow w-80 max-w-80">
        {/* Header indicator */}
        <div className="bg-blue-50 px-3 py-1 rounded-t-lg border-b border-blue-100">
          <span className="text-xs font-medium text-blue-700">PROSJEKTKRAV</span>
        </div>
...
export default ProsjektKravFlowNode;