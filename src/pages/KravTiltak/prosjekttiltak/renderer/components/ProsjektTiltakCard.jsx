...
/**
 * ProsjektTiltakCard - Entity-specific card for ProsjektTiltak
 * 
    ...
    >
      ...
      <div className="flex items-center justify-between gap-2 mb-1">
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          {/* General Tiltak reference */}
          {entity.generalTiltak && viewOptions.showGeneralTiltak && (
            <span className="text-xs text-purple-600 font-medium flex items-center gap-1">
              {getIcon("Link", 10)}
              {entity.generalTiltak.tiltakUID} - {truncateText(entity.generalTiltak.tittel, 12)}
            </span>
          )}
          ...
          {entity.parentId && entity.parent && viewOptions.showHierarchy && (
            <span className="text-xs text-purple-600 font-medium">
              ↑ {entity.parent.tiltakUID} - {truncateText(entity.parent.tittel, 10)}
            </span>
          )}
        </div>
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          {viewOptions.showObligatorisk && entity.obligatorisk !== undefined && (
            <div 
              className={entity.obligatorisk ? 'text-purple-600' : 'text-green-600'} 
        ...
        <div className="flex items-center gap-3">
          ...
          {viewOptions.showRelations && entity.children?.length > 0 && (
            <span className="text-emerald-600 font-medium">
              {entity.children.length} undertiltak
            </span>
          )}
          
          {/* ProsjektKrav connections */}
          {viewOptions.showRelations && entity.prosjektKrav?.length > 0 && (
            <span className="text-blue-600 font-medium">
              ← {entity.prosjektKrav.length} krav
            </span>
          )}
