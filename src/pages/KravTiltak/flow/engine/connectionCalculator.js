...
export function preCalculateAllConnections(allKravEntities, allTiltakEntities, globalRelationships) {
  ...
  allEntities.forEach(({ type, entity }) => {
    ...
    const parentRelation = globalRelationships.kravToKrav.find(rel => rel.child.id === entity.id) ||
    ...
    if (parentRelation) {
      ...
      const parentNodeKey = isParentKrav 
        ? `krav-${parentRelation.parent.id}` 
        : `tiltak-${parentRelation.parent.id}`;
    ...
    if (entityTypeLower === 'prosjekttiltak' || entityTypeLower === 'tiltak') {
      ...
      console.log(`[LOGBACKEND] PRE-CALC ${nodeKey}: found ${connectedKrav.length} krav connections`);
      
      connectedKrav.forEach((rel) => {
        console.log(`[LOGBACKEND] PRE-CALC ${nodeKey}: adding krav-${rel.krav.id}`);
