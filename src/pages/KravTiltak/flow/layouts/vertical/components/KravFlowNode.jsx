...
/**
 * KravFlowNode (Vertical Layout) - React Flow node for Krav entities
 * Both handles on left middle edge for clean vertical flow
 */
const KravFlowNode = ({ data, selected }) => {
  ...
  return (
    <>
      {hasIncoming && <Handle type="target" position={Position.Left} id="krav-target" style={{ background: "#3b82f6", top: '50%' }} />}
      {hasOutgoing && <Handle type="source" position={Position.Left} id="krav-source" style={{ background: "#3b82f6", top: '50%' }} />}
      ...
      <div className="bg-white border-2 border-gray-200 rounded-lg shadow-sm hover:shadow-md transition-shadow w-80 max-w-80">
        ...
        <div className="bg-blue-50 px-3 py-1 rounded-t-lg border-b border-blue-100 flex justify-between items-center">
          ...
          {dto && entity && <span className="text-xs font-mono text-blue-600">{dto.extractUID(entity)}</span>}
        </div>
