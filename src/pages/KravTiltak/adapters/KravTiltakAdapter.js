...
export class KravTiltakAdapter {
  constructor(kravConfig, tiltakConfig = null, options = {}) {
    ...
    this.baseAdapter = new EntityWorkspaceAdapter();
  }
  ...
  getDisplayConfig() {
    return {
      ...
      entityTypes: this.isCombinedView ? ['krav', 'tiltak'] : ['krav'],
  ...
  /**
   * Transform API response using base adapter + domain enhancements
   */
  transformResponse(rawData, entityType = 'krav') {
    const transformed = this.baseAdapter.transformResponse(rawData);
    
    if (transformed.items) {
      transformed.items = transformed.items.map(entity => 
        this.enhanceEntity(entity, entityType)
      );
    }
  ...
  enhanceEntity(entity, entityType) {
    return {
      ...
      // UI display helpers
      displayType: this.getDisplayType(entity.entityType || entityType),
      badgeColor: this.getBadgeColor(entity.entityType || entityType),
    };
  }
