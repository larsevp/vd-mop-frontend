...
export class EntityWorkspaceAdapter extends BaseAdapter {
  ...
  transformGroupedResponse(rawData) {
    console.log('EntityWorkspaceAdapter: transformGroupedResponse - preserving grouped structure');
    
    // Transform each group while preserving the grouped structure
    const transformedGroups = rawData.items.map((group, groupIndex) => {
      ...
      const transformedEntities = entities.map(entity => {
        ...
        // Add emne information to each entity for backward compatibility
        if (emne) {
      ...
      // Preserve the original group structure while using transformed entities
      const transformedGroup = {
        emne: emne,
        ...group // Keep original keys (krav, prosjektkrav, etc.)
      };
      
      // Replace entity arrays with transformed versions
      const entityKeys = ['entities', 'krav', 'tiltak', 'prosjektkrav', 'prosjekttiltak'];
      entityKeys.forEach(key => {
        if (group[key]) {
          transformedGroup[key] = transformedEntities;
        }
      });
    ...
    // Return grouped structure instead of flattened
    return {
      items: transformedGroups, // Preserve grouped structure
      ...
      isGrouped: true,
      metadata: {
        source: 'EntityWorkspaceAdapter.transformGroupedResponse',
        groups: transformedGroups.length,
        totalEntities: transformedGroups.reduce((sum, group) => {
          const entityKeys = ['entities', 'krav', 'tiltak', 'prosjektkrav', 'prosjekttiltak'];
          return sum + entityKeys.reduce((groupSum, key) => groupSum + (group[key]?.length || 0), 0);
        }, 0)
      }
    };
  }
