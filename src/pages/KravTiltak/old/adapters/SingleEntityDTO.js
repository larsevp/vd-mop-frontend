...
export class SingleEntityDTO {
  constructor(adapter, options = {}) {
    ...
    console.log('SingleEntityDTO constructor:', {
      entityType: this.entityType,
      displayConfig: this.adapter.getDisplayConfig()
    });
  }
      ...
      if (transformedData.isGrouped && transformedData.items) {
        ...
        transformedData.items = transformedData.items.map(groupData => ({
          ...
          items: groupData.items.map(entity => 
            this.adapter.enhanceEntity(entity, this.entityType)
          )
        }));
  ...
  _transformGroupedResponse(rawData) {
    ...
    console.log('_transformGroupedResponse:', { propertyName, rawDataItems: rawData.items });
    
    const normalizedGroups = rawData.items.map(group => {
      const groupItems = group[propertyName] || group.entities || [];
      console.log('Processing group:', { 
        emne: group.emne?.tittel, 
        propertyName, 
        hasProperty: !!group[propertyName],
  ...
  async loadData(queryParams = {}) {
    ...
    try {
      ...
      console.log(`SingleEntityDTO[${this.entityType}]: Query functions:`, queryFunctions);
      
      // For individual entities, use the grouped query function
      const queryFn = queryFunctions[this.entityType]?.grouped;
      
      if (!queryFn) {
        console.error(`SingleEntityDTO[${this.entityType}]: No query function found for entity type. Available:`, Object.keys(queryFunctions));
      ...
      console.log(`SingleEntityDTO[${this.entityType}]: About to transform response`);
      const transformedResponse = this.transformResponse(rawData);
      console.log(`SingleEntityDTO[${this.entityType}]: Transform completed:`, {
        isGrouped: transformedResponse.isGrouped,
