...
const tiltak = [
  { id: 1, tittel: 'Sjekk brannslukker', beskrivelse: 'Kontroller at brannslukker er på plass og i orden', temaId: 2, implementasjon: 'Utført månedlig', avklaringer: 'Ingen', vurdering: ['Bra'] },
  { id: 2, tittel: 'Vernerunde', beskrivelse: 'Gjennomfør vernerunde i alle avdelinger', temaId: 1, implementasjon: '', avklaringer: '', vurdering: [] },
];
...
export default function TiltaksoversiktProsjekt() {
  ...
  const filteredTemaer = selectedTema 
    ? temaer.filter(t => t.id === Number(selectedTema)) 
    : temaer;
    
  const filteredTiltak = searchTerm
    ? tiltakState.filter(t => t.tittel.toLowerCase().includes(searchTerm.toLowerCase()) || 
      ...
      <main className="py-6">
        ...
        {filteredTemaer.length === 0 && (
          <div className="text-center py-20 text-neutral-500">
            Ingen tema funnet med det valgte filteret.
          </div>
        )}
        
        {filteredTemaer.length > 0 && filteredTiltak.length === 0 && (
          <div className="text-center py-20 text-neutral-500">
            Ingen tiltak funnet som matcher søket.
          </div>
        )}
      </main>
    </div>
