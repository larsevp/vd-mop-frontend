...
const Icon = React.forwardRef<React.ComponentRef<LucideIcon>, IconProps>(({ name, ...props }, ref) => {
  // Convert PascalCase to kebab-case for lucide-react/dynamic compatibility
  const kebabCaseName = name
    ?.replace(/([A-Z])/g, "-$1")
    .toLowerCase()
    .replace(/^-/, "") as IconName; // Remove leading dash

  try {
    return <DynamicIcon name={kebabCaseName} {...props} ref={ref} />;
  } catch (error) {
    // Suppress lucide-react dynamic icon errors since our conversion logic handles them properly
    // Just return a fallback circle icon if something goes wrong
    return <DynamicIcon name="circle" {...props} ref={ref} />;
  }
});
