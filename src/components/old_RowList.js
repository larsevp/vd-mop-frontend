...
export const modelConfigs = {
  ...
  enheter: {
    ...
    fields: [
      ...
      { name: "parentId", label: "Parent", type: "number", required: false, hiddenIndex: true },
      ...
      },
      {
        ...
        type: "text",
        required: false,
        computed: (row) => row.parent?.name || "N/A", // Extracts parent.name or defaults to "N/A"
      },
    ],
  },
};
...
function displayFieldValue(field, row) {
  if (field.computed) {
    return field.computed(row); // Evaluate the computed function if it exists
  }
  return row[field.name] || ""; // Fallback to the field's direct value
}

// Example usage in RowList
function RowList({ rows, fields }) {
  return (
    <table>
      <thead>
        <tr>
      ...
      <tbody>
        {rows.map((row) => (
          <tr key={row.id}>
            {fields.map((field) => (
              <td key={field.name}>{displayFieldValue(field, row)}</td>
            ))}
          </tr>
        ))}
      </tbody>
