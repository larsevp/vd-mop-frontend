...
class BaseStateHandler {
  ...
  async rollback(context, stateManager) {
    ...
    console.log(`No rollback implemented for ${this.operation}`);
  }
}
...
class WorkspaceSwitchHandler extends BaseStateHandler {
  ...
  async handle(context, stateManager) {
    ...
    console.log('WorkspaceSwitchHandler: Switching workspace', {
      from: fromEntityType,
      to: toEntityType,
      hasDTO: !!dto
    });
...
class StoreResetHandler extends BaseStateHandler {
  ...
  async handle(context, stateManager) {
    ...
    console.log('StoreResetHandler: Resetting stores', { entityTypes, clearCache });
...
class CacheClearHandler extends BaseStateHandler {
  ...
  async handle(context, stateManager) {
    ...
    console.log('CacheClearHandler: Clearing cache', { cacheTypes, entityTypes });
    ...
    if (cacheTypes.includes('all') || cacheTypes.includes('query')) {
      ...
      console.log('CacheClearHandler: Query cache clearing would go here');
    }
    ...
    subscribeWithSelector((set, get) => ({
      ...
      initialize: () => {
        ...
        if (state.initialized) {
          console.log('StateHandler: Already initialized, skipping');
          return;
        }
        ...
        console.log('StateHandler: Initialized with handlers', Array.from(handlers.keys()));
      },
      ...
      registerHandler: (handler) => {
        ...
        console.log('StateHandler: Registered handler', handler.operation);
      },
      ...
      execute: async (operation, payload = {}) => {
        ...
        if (state.isProcessing) {
          console.log('StateHandler: Operation already in progress, queuing...');
        ...
        try {
          ...
          // Execute handler
          console.log('StateHandler: Executing operation', { operation, id: context.id });
          ...
          // Record operation
          set(prevState => ({
            operationHistory: [...prevState.operationHistory.slice(-49), { // Keep last 50
              context,
              result,
              timestamp: Date.now()
            }],
